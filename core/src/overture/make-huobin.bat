...
SET BUILD=1
IF EXIST "%TARGET%\armeabi-v7a\libhuobin.so" (
	IF EXIST "%TARGET%\arm64-v8a\libhuobin.so" (
		IF EXIST "%TARGET%\x86\libhuobin.so" (
			SET BUILD=0
		)
	)
)
...
IF %BUILD% == 1 (
	ECHO "Get dependences for libhuobin"
	go.exe get -u github.com\tools\godep

	PUSHD %GOPATH%\src\huobin.cn\src
	godep.exe restore

	ECHO "Cross compile libhuobin for arm"
	IF NOT EXIST "%TARGET%\armeabi-v7a\libsss-proxy.so" (
	    ...
	    %ANDROID_ARM_STRIP% src
	    MOVE src %TARGET%\armeabi-v7a\libhuobin.so>nul 2>nul 		
	    ENDLOCAL
	)

	ECHO "Cross compile libhuobin for arm64"
	IF NOT EXIST "%TARGET%\arm64-v8a\libsss-proxy.so" (
	    ...
	    SET GOARCH=arm64
	    go.exe build -ldflags="-s -w"
	    %ANDROID_ARM64_STRIP% src
	    MOVE src %TARGET%\arm64-v8a\libhuobin.so>nul 2>nul 
	    ENDLOCAL
	)

	ECHO "Cross compile libhuobin for x86"
	IF NOT EXIST "%TARGET%\x86\libhuobin.so" (
		SETLOCAL
	    SET CGO_ENABLED=1
	    ...
	    SET GOARCH=386
	    go.exe build -ldflags="-s -w"
	    %ANDROID_X86_STRIP% src
	    MOVE src %TARGET%\x86\libhuobin.so>nul 2>nul 
	    ENDLOCAL
	)
