...
SET BUILD=1
IF EXIST "%TARGET%\armeabi-v7a\libsss-proxy.so" (
	IF EXIST "%TARGET%\arm64-v8a\libsss-proxy.so" (
		IF EXIST "%TARGET%\x86\libsss-proxy.so" (
			SET BUILD=0
		)
	)
)
...
IF %BUILD% == 1 (
	ECHO "Get dependences for libsss-proxy"
	go.exe get -u github.com\tools\godep

	PUSHD %GOPATH%\src\github.com\shadowsocks\shadowsocks-go\cmd\shadowsocks-server
	godep.exe restore

	ECHO "Cross compile libsss-proxy for arm"
	IF NOT EXIST "%TARGET%\armeabi-v7a\libsss-proxy.so" (
		SETLOCAL
	    SET CGO_ENABLED=1
	    ...
	    %ANDROID_ARM_STRIP% shadowsocks-server
	    MOVE shadowsocks-server %TARGET%\armeabi-v7a\libsss-proxy.so>nul 2>nul 		
	    ENDLOCAL
	)

	ECHO "Cross compile libsss-proxy for arm64"
	IF NOT EXIST "%TARGET%\arm64-v8a\libsss-proxy.so" (
		SETLOCAL
	    SET CGO_ENABLED=1
	    ...
	    SET GOARCH=arm64
	    go.exe build -ldflags="-s -w"
	    %ANDROID_ARM64_STRIP% shadowsocks-server
	    MOVE shadowsocks-server %TARGET%\arm64-v8a\libsss-proxy.so>nul 2>nul 
	    ENDLOCAL
	)

	ECHO "Cross compile libsss-proxy for x86"
	IF NOT EXIST "%TARGET%\x86\libsss-proxy.so" (
		SETLOCAL
	    SET CGO_ENABLED=1
	    ...
	    SET GOARCH=386
	    go.exe build -ldflags="-s -w"
	    %ANDROID_X86_STRIP% shadowsocks-server
	    MOVE shadowsocks-server %TARGET%\x86\libsss-proxy.so>nul 2>nul 
	    ENDLOCAL
	)
...
ECHO "Successfully build libsss-proxy target:%TARGET%"
ENDLOCAL
